...
/** @param {NS} ns **/
export async function explore(ns: NS, server: string, oldServer = "", list: ServerInfo[] = []): Promise<ServerInfo[]> {
...
export function getServerInfo(ns: NS, host: string): ServerInfo {
  if (!host) throw "ns and host not populated for getServerInfo";
  const purchased = ns.getPurchasedServers();
  const server = new ServerInfo();
  server.hostname = host;
  server.isAdmin = ns.hasRootAccess(host);
  server.hackLevel = ns.getServerRequiredHackingLevel(host);
  server.rootPorts = ns.getServerNumPortsRequired(host);
  server.securityLevel = ns.getServerSecurityLevel(host);
  server.minSecurityLevel = ns.getServerMinSecurityLevel(host);
  server.growthFactor = ns.getServerGrowth(host);
  server.money = ns.getServerMoneyAvailable(host);
  server.maxMoney = ns.getServerMaxMoney(host);
  server.ram = ns.getServerMaxRam(host);
  server.isPurchased = purchased.includes(host);
  return server;
}
...
export function printServerSummary(ns: NS, serverInfo: ServerInfo): void {
  ns.tprintf("%20s: Money - %10s(%10s)  |  Security - %8s(%8s)", serverInfo.hostname, ns.nFormat(serverInfo.money, "$0.000 a"), ns.nFormat(serverInfo.maxMoney, "$0.000 a"), ns.nFormat(serverInfo.securityLevel, "0.000"), ns.nFormat(serverInfo.minSecurityLevel, "0.000"))
}

/** @param {NS} ns **/

export function isHackable(ns: NS, serverInfo: ServerInfo): boolean {
  return serverInfo.maxMoney > 0 && serverInfo.hackLevel <= ns.getHackingLevel() && serverInfo.isAdmin
}
...
/** @param {NS} ns **/
export function killProcesses(ns: NS, serverInfo: ServerInfo): boolean {
  if (!serverInfo.isAdmin) return false;
...
/** @param {NS} ns **/
/** @param {ServerInfo} host **/
export async function populateServer(ns: NS, server: string | ServerInfo): Promise<boolean> {
  let host = ""
...
export function analyzeTarget(ns: NS, serverInfo: ServerInfo): void {
  ...
  const hackThreads = ns.hackAnalyzeThreads(serverInfo.hostname, serverInfo.money / 2);
  ...
  const growMaximize = ns.growthAnalyze(serverInfo.hostname, serverInfo.maxMoney / serverInfo.money);
...
export function getRatios(ns: NS, target: string | ServerInfo, hackThreads = 1): ThreadRatios {
...
export async function maximizeRatios(ns: NS, target: ServerInfo, host: ServerInfo, printInfo = false): Promise<ThreadRatios> {
  if (printInfo) ns.tprintf("Maximizing for %s from %s", target.hostname, host.hostname);
  const threadLimit = (host.ram / 1.75) - 1;
  const baseline = getRatios(ns, target, 1);

  let mark = getRatios(ns, target, Math.ceil(ns.hackAnalyzeThreads(target.hostname, target.money * .5)));
