export function hammingCodesIntegerToBinary(value: string) {
  // encoding following Hammings rule
  function HammingSumOfParity(_lengthOfDBits: number) {
  ...
  for (const index of _build.reduce(function (a:number[], e:string, i:number) {
    if (e == "x") a.push(i);
    return a;
  }, [])) {
...
export function hammingCodesBinaryToInteger(data: string) {
  //check for altered bit and decode
  const _build = data.split(""); // ye, an array for working, again
  const _testArray = []; //for the "truthtable". if any is false, the data has an altered bit, will check for and fix it
  const _sumParity = Math.ceil(Math.log2(data.length)); // sum of parity for later use
  const count = (arr: string[], val: string) =>
      arr.reduce((a, v) => (v === val ? a + 1 : a), 0);
  // the count.... again ;)
